<template>
	<view>
		<u-navbar :is-back="true" title="绑定的设备清单" title-color="#fff" :border-bottom="false" :background="background"
		 back-icon-color="#fff">
			<view class="flex align-center  w-100">
				<view class="position-absolute" style="right: 30rpx;">
					<u-button hover-class="none" :custom-style="customStyle" plain size="mini" :hair-line="false" @click="handleBtnClick">查看设备信息</u-button>
				</view>
			</view>
		</u-navbar>
		<view class="w-100 flex justify-center flex-column" v-if="list.length">
			<view class="w-100 flex justify-center mt-2" v-for="(item,index) in list" :key="index">
				<view @click="handleItem(index)" :class="{active:index == isActive}" class="flex align-center rounded border bg-white"
				 style="width: 686rpx;height: 240rpx;">
					<view class="ml-2" style="width: 30%;height: 200rpx;">
						<u-image border-radius="8rpx" width="100%" height="200rpx" :src="item.e_type == '胎心监护仪' ? imgList[0] : item.e_type == '血糖仪' ? imgList[1] : item.e_type == '血压仪' ? imgList[2] : ''"></u-image>
					</view>
					<view class="flex flex-column ml-2" style="width: 70%;">
						<text>设备名称：{{item.e_name}}</text>
						<text>设备类型：{{item.e_type}}</text>
						<text :style="item.status == '已领用' ? 'color:#71d5a1' : 'color:#c8c9cc'">状态：{{item.status}}</text>
						<!-- <text>异常：无</text> -->
					</view>
				</view>
			</view>
