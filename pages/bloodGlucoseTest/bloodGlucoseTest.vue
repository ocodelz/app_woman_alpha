<template>
	<view class="flex flex-column" :style="'height:' + pageHeight + 'px'">
		<u-navbar title="血糖测量" title-color="#fff" back-icon-color="#fff" :background="background" :border-bottom="false"></u-navbar>
		<!-- 测量结果正文 -->
		...
		<!-- 设备连接状态 -->
		<!--  -->
		<view class=" justify-center w-100 flex flex-row " style="margin-top: 15rpx;" v-if="tapState == 0">
			<text class="font-md ml-1" :style="equipmentStatus == true ? 'color:#19be6b' : 'color:#f00'">{{ deviceStatus }}</text>
		</view>
		<!-- 保存按钮 -->
		...
		<u-popup mode="center" v-model="show" width="700" border-radius="12" safe-area-inset-bottom v-if="purchaseState == 0">
