<template>
	<view class="content">
		<u-navbar :is-back="false" title="监护记录" title-color="#fff" :border-bottom="false" :background="background"
		 back-icon-color="#fff">
			<view class="flex align-center  w-100">
				<view class="flex align-center ml-2" style="width: 140rpx;height: 80rpx;" @click="handleTapBack">
					<u-icon @click="handleTapBack" color="#fff" name="back" custom-prefix="custom-icon" size="44"></u-icon>
				</view>
		...
		<view class="nav-box">
			<view class="nav">
				<view class="nav-item" v-for="(item,index) in navList" :key="index" @tap="navTap(index)" :class="currentTab == index?'active':''">{{item}}</view>
			</view>
		...
		<swiper class="swiper" disable-touch :current="currentTab">
			<swiper-item v-for="(itemL,indexL) in list" :key="indexL">
				<scroll-view scroll-y="true" style="height: 100%;" @touchstart="start" @touchend="end">
					<uni-swipe-action>
						<uni-swipe-action-item v-for="(item,index) in itemL" :key="index" :right-options="item.state == 1 ? options2 : null"
						 :show="isOpened" :auto-close="true" @click="bindClick($event,item,index)">
							<view @click="handleItemClick(item)" class="cell-box" :style="item.state == 1 ? 'background:#d9d9d9;' : 'background:#ffffff'">
								...
								<view class="cell" v-show="item.type == '血压'">{{item.low_pressure}} - {{item.high_pressure}}</view>
		...
		<free-popup ref="extend">
			<view class="bg-white position-fixed flex align-center pl-3" style="border: 1rpx solid #dee2e6; border-radius: 12rpx; right: 120rpx; top: 140rpx; width: 200rpx;height: 200rpx;">
				<u-checkbox-group>
	...
	export default {
		...
		methods: {
			...
			handleGetQueryYfclInfo(_arr) {
				...
				this.$u.post('/QueryYFCLInfo2', data).then(res => {
					// console.log(res);
					this.disabledTouch = res.data.flag;
